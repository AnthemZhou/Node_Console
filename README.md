...
![Version](https://img.shields.io/badge/version-0.8.34-blue)
...
1. 从 release assets 下载 `Node_Console_0.8.34.zip`。
...
- `启用中文模糊检索`（Enable Chinese Fuzzy Match）：开启中文跳词模糊检索。默认关闭，因为它可能让实时搜索略微变慢。
- `搜索窗口大小`（Console Size）：调整搜索窗口整体尺寸，范围是 0.5 到 2.0。
- `显示缓存的资产节点`（Show Cached Asset Nodes）：在搜索结果中显示已缓存的资产节点组。
- `显示类目颜色标签`（Show Category Color Tags）：显示或隐藏搜索结果左侧的类目颜色底块。
...
1. Download `Node_Console_0.8.34.zip` from the release assets.
...
- `Show Category Color Tags`: shows or hides colored category backgrounds on the left side of search results.
