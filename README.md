...
Node Console 是一个 Blender 节点搜索插件。它会替换节点编辑器里的 `Shift + A` 搜索，让你可以用英文、中文或拼音搜索节点。界面保持紧凑，视觉风格接近 Blender 原生搜索。

### 功能

- 用自定义搜索窗口替换节点编辑器中的 `Shift + A`。
- 搜索内置 Shader、Geometry、Compositor 和 Texture 节点。
- 不依赖当前 Blender 界面语言。
- 支持英文、中文、中英双语和拼音搜索。
- macOS 优先使用系统拼音转换。
- Windows 和 Linux 使用插件内置拼音兜底表。
- 可选开启中文模糊检索，例如用 `设置法向` 匹配 `设置曲线法向`。
- 在搜索结果左侧显示节点类型颜色标签。
- 支持收藏节点和快捷节点。
- 在搜索框下方显示快捷节点，并支持一键创建。
...
Node Console is a custom node search add-on for Blender. It replaces the default `Shift + A` search in the Node Editor. You can search nodes by English name, Chinese name, or pinyin, with a compact layout inspired by Blender native search.

### Features

- Replaces `Shift + A` in the Node Editor with a custom search panel.
- Searches built-in Shader, Geometry, Compositor, and Texture nodes.
- Works independently of the current Blender interface language.
- Supports English, Chinese, bilingual display, and pinyin search.
- Uses macOS system pinyin conversion when available.
- Uses a bundled pinyin fallback table on Windows and Linux.
- Offers optional fuzzy matching for sparse queries, for example `set normal` matching `Set Curve Normal`.
- Shows colored node type tags on the left side of search results.
- Supports favorite nodes and shortcut nodes.
- Shows shortcut nodes under the search field for quick creation.
- Supports manually refreshed Asset Library node group cache.
- Keeps the newly created node selected and lets it follow the mouse before placement.
- Saves display mode, favorites, shortcuts, and asset cache in a separate settings file.
